...
1. **Kies een release en een livebranch.** De werkmap bevat nog steeds veel niet-gecommitte wijzigingen naast de "Eenvoudig publiceren"-commits op `release/eenvoudig-publiceren`. Review welke wijzigingen bij de live versie horen, maak een schone, herkenbare releasecommit, en kies pas dan de branchnaam die `render.yaml` (`branch: live`) en de hostingprovider echt gaan gebruiken.
